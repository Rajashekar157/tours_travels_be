...
class DriverCreate(BaseModel):
    # driver_code is NOT accepted from the client anymore — it's
    # auto-generated in create_driver_service() as DRV-000001, DRV-000002, ...
    # via a Postgres sequence, so it's guaranteed unique with no race condition.
...
class DriverUpdate(BaseModel):
    ...
    # driver_code intentionally NOT editable — it's a permanent generated
    # identifier, same reasoning as not letting users edit a primary key.
